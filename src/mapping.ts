...
function handleAction(
  ...
  //let sender_id = receiptWithOutcome.receipt.predecessorId.toString()
  //let contract_id = receiptWithOutcome.receipt.receiverId.toString()
  //let receiver_id = receiptWithOutcome.receipt.signerId.toString()
  ...
  if (methodName == 'create_twitter_pool' || methodName == 'update_twitter_pool' ) {
    for (let logIndex = 0; logIndex < outcome.logs.length; logIndex++) {
      ...
      log.info('outcomeLog Create & update {}', [outcomeLog])
      if (outcomeLog.includes('EVENT:')){
        ...
        for(let i=0;i<ft_prizes.length;i++){
            ...
            if (!activity){
              ...
              activity.contract_id!= "prizepool.superise.testnet"
              activity.sender = creator_id
              activity.receiver = "prizepool.superise.testnet"
  ...
  if (methodName == 'publish_pool'){
    for (let logIndex = 0; logIndex < outcome.logs.length; logIndex++) {
      let outcomeLog = outcome.logs[logIndex].toString();
      log.info('publish outcomeLog {}', [outcomeLog])
      if (outcomeLog.includes('EVENT:')){
        outcomeLog = outcomeLog.replace('EVENT:','')
        const jsonData = json.try_fromString(outcomeLog)
        const jsonObject = jsonData.value.toObject()
  ...
  if (methodName == 'withdraw_ft'){
    for (let logIndex = 0; logIndex < outcome.logs.length; logIndex++) {
      let outcomeLog = outcome.logs[logIndex].toString();
      log.info('withdraw ft outcomeLog {}', [outcomeLog])
      if (outcomeLog.includes('EVENT:')){
        outcomeLog = outcomeLog.replace('EVENT:','')
        const jsonData = json.try_fromString(outcomeLog)
        const jsonObject = jsonData.value.toObject()
        ...
        if (!activity){
          ...
          activity.sender = "prizepool.superise.testnet"
          activity.receiver = account_id.toString()
          activity.contract_id = "prizepool.superise.testnet"
          activity.msg = "Withdraw from superise: " + token_id.toString()
          activity.type = "FT"
          activity.method = methodName
          let ft = new FT(tx_hash.toBase58()+"<FT>"+account_id.toString())
          ft.token_id = token_id.toString()
          ft.amount = amount.toString()
          ft.save()
          activity.ft = tx_hash.toBase58()+"<FT>"+account_id.toString()
          activity.save()
        }
      }
    }
  ...
  if (methodName == 'withdraw_nft'){
    for (let logIndex = 0; logIndex < outcome.logs.length; logIndex++) {
      let outcomeLog = outcome.logs[logIndex].toString();
      log.info('withdraw nft outcomeLog {}', [outcomeLog])
      if (outcomeLog.includes('EVENT:')){
        outcomeLog = outcomeLog.replace('EVENT:','')
        const jsonData = json.try_fromString(outcomeLog)
        const jsonObject = jsonData.value.toObject()
        ...
        if (!activity){
          ...
          activity.type = "NFT"
          activity.method = methodName
          let nft = new NFT(tx_hash.toBase58()+"<NFT>"+account_id.toString())
          nft.contract_id = nft_contract_id.toString()
          nft.nft_id = nft_id.toString()
          nft.save()
          activity.ft = tx_hash.toBase58()+"<NFT>"+account_id.toString()
          activity.save()
        }
      }
    }
  ...
  if (methodName == 'touch_pools' ) {
    ...
    for (let logIndex = 0; logIndex < outcome.logs.length; logIndex++) {
      let outcomeLog = outcome.logs[logIndex].toString();
      log.info('touch outcomeLog {}', [outcomeLog])
      if (outcomeLog.includes('EVENT:')){
        outcomeLog = outcomeLog.replace('EVENT:','')
        const jsonData = json.try_fromString(outcomeLog)
        const jsonObject = jsonData.value.toObject()
